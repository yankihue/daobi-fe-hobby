  ...
  useEffect(() => {
    async function fetchData() {
      ...
      for (const event of response.data.result) {
        ...
        if (topics[0] === '0xa629d669aa8e26d0998c7d6a40ad225ddea112e0c54474a08ef6c039bd302476') {
        ...
        } else if (topics[0] === '0xce0c7a2a940807f7dc2ce7a615c2532e915e6c0ac9a08bc4ed9d515a710a53e2') {
          const fromAddr = topics[1];
          users[fromAddr] = [fromAddr, users[fromAddr][1], topics[2]];
        } else if (topics[0] === '0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925') {
          const fromAddr = topics[1];
          if (fromAddr in users) {
            delete users[fromAddr];
          }
        } else if (topics[0] === '0xe2795580f31af61313dfda02eec54a3b17d06e395138a74e448cb4cb75d3d818') {
          const fromAddr = topics[1];
      ...
      <table>
        ...
        <tbody>
          {Object.keys(factions).map((factionName) => (
            <tr key={factionName}>
              <td>{factionName}</td>
              <td>{factions[factionName]['votes']}</td>
              <td>{factions[factionName]['voters'].join(", ")}</td>
            </tr>
          ))}
        </tbody>
      </table>
